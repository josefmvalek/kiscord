...
/**
 * Vrátí rychlý přehled dnešních událostí pro Pinned Calendar Super-Channel
 * @returns {{ todayKey: string, formattedDate: string, eventsCount: number, summary: string }}
 */
export function getTodayEventsSummary() {
    ...
    // 1. Deadlines & zkoušky VUT FIT
    const todayDeadlines = (state.schoolDeadlines || []).filter(dl => dl.deadline_date === todayKey);
    const todayStudy = (state.studyPlannerItems || []).filter(item => item.dueDate === todayKey && !item.completed);
    const totalSchool = todayDeadlines.length + todayStudy.length;
    eventsCount += totalSchool;
    if (totalSchool > 0) {
        highlightText = `${totalSchool} ${totalSchool === 1 ? 'škola/zkouška' : 'školní události'}`;
    }

    // 2. Rande a společné plány
    ...
    const totalDates = todayPlans.length + todayDates.length;
    eventsCount += totalDates;
    if (totalDates > 0 && !highlightText) {
        highlightText = `${totalDates} ${totalDates === 1 ? 'společný plán' : 'společné plány'}`;
    }

    // 3. Směny
    const todayShifts = (state.shiftsSchedule || []).filter(s => s.date_key === todayKey);
    eventsCount += todayShifts.length;
    if (todayShifts.length > 0 && !highlightText) {
        highlightText = 'Dnes je směna';
    }

    // 4. Gym tréninky
    const todayGym = (state.gymLogs || []).filter(g => g.date_key === todayKey);
    if (todayGym.length > 0) {
        eventsCount += todayGym.length;
        if (!highlightText) highlightText = 'Trénink v posilovně';
    }
    ...
    return {
        ...
        summary: eventsCount > 0 
            ? `Dnes: ${eventsCount} ${eventsCount === 1 ? 'událost' : (eventsCount < 5 ? 'události' : 'událostí')}${highlightText ? ` • ${highlightText}` : ''}`
