...
export function renderList(container, subject) {
    ...
    Promise.all([
        import('@domains/entertainment/spaced-repetition.js'),
        import('@domains/system/progress.js')
    ]).then(async ([sr, pr]) => {
        for (const item of items) {
            ...
            if (rTextEl && rBarEl) {
                ...
                if (total > 0) {
                    ...
                    if (item.has_content) {
                        import('@domains/university/matura/index.js').then(m => m.silentBackfillCount(item.id));
                    }
