...
import { BRNO_CAMPUS_FOOD } from '@domains/university/dorm-hub.js';
...
export function openDecisionMatcher(mode = 'food', category = 'all') {
    matcherMode = mode;
...
function prepareDeck() {
    if (matcherMode === 'food') {
        ...
        const locationList = (state.dateLocations || [])
            .filter(l => l.cat === 'food' || l.category === 'Jídlo' || l.category === 'Kavárna' || l.category === 'Bistro')
            .map(l => ({
                id: l.id || l.name,
                title: l.name,
                tag: l.category || 'Gastro',
                desc: l.desc || l.address || 'Oblíbený podnik v Brně',
                icon: 'fa-coffee',
        ...
        const placesDeck = (state.dateLocations || [])
            .filter(p => activeCategoryFilter === 'all' || p.cat === activeCategoryFilter)
            .map(p => ({
                id: p.id,
                title: p.name,
                tag: p.cat === 'view' ? '⛰️ Výhled' : (p.cat === 'food' ? '🍔 Jídlo' : (p.cat === 'walk' ? '🌲 Příroda' : '⚡ Zábava')),
                desc: p.desc || p.address || 'Krásné místo na výlet či procházku 🌲',
                icon: p.cat === 'food' ? 'fa-utensils' : (p.cat === 'view' ? 'fa-binoculars' : 'fa-map-marker-alt'),
...
function renderMatcherModal() {
    ...
    const title = matcherMode === 'food' ? 'Kam dnes na oběd či večeři? 🍕' : '💖 Rande Matcher (Tinder pro rande)';
    const subtitle = matcherMode === 'food' ? 'Tinder pro menzy a restaurace u FITu & v Brně' : 'Inspirace z mapy míst a bucket listu';

    const card = currentDeck[currentCardIndex];

    const contentHtml = `
        <div id="matcher-card-container" class="min-h-[340px] flex flex-col items-center justify-between p-2 select-none">
            ${card ? `
                <div class="w-full bg-gradient-to-br ${card.color} border rounded-3xl p-6 shadow-2xl space-y-4 text-center transform transition-all duration-300 animate-fade-in relative overflow-hidden" id="active-swipe-card">
                    ...
                    ${card.image_url ? `
                        <div class="w-full h-36 rounded-2xl overflow-hidden mb-2 shadow-inner">
                ...
                <div class="flex items-center justify-center gap-6 w-full pt-4">
                    <button onclick="window.handleSwipeDecision(false)" 
                            class="w-14 h-14 rounded-2xl bg-rose-500/15 hover:bg-rose-500/25 text-rose-400 border border-rose-500/30 flex items-center justify-center text-xl shadow-lg transition transform active:scale-90 hover:scale-105"
                    ...
                    <button onclick="window.handleSwipeDecision(true)" 
                            class="w-14 h-14 rounded-2xl bg-emerald-500/15 hover:bg-emerald-500/25 text-emerald-400 border border-emerald-500/30 flex items-center justify-center text-xl shadow-lg transition transform active:scale-90 hover:scale-105"
                            title="To beru!">
                        <i class="fas fa-heart"></i>
                    </button>
                </div>
                <div class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-2">
            ...
            ` : `
                <div class="text-center py-8 space-y-4">
                    ...
                    ${likedCards.length > 0 ? `
                        <div class="p-3 bg-emerald-500/10 border border-emerald-500/30 rounded-2xl text-left">
                            <p class="text-xs text-emerald-400 font-bold">Vybraná místa: ${likedCards.map(c => c.title).join(', ')}</p>
                        </div>
                    ` : '<p class="text-xs text-gray-400">Zkuste balíček zamíchat znovu.</p>'}
                    <button onclick="window.openDecisionMatcher('${matcherMode}')" 
                            class="px-6 py-2.5 rounded-xl bg-[#5865F2] hover:bg-[#4752c4] text-white font-bold text-xs uppercase tracking-wider transition shadow">
...
export function handleSwipeDecision(liked) {
    ...
    if (liked) {
        ...
        showNotification(`Líbí se vám: ${card.title}! 🎉`, 'success');
    } else {
        triggerHaptic('light');
    }
