...
function showMatchWinnerScreen(item) {
    ...
    overlay.innerHTML = `
        <div class="text-center space-y-6 max-w-sm w-full animate-scale-up">
            ...
            <div class="flex flex-col gap-2.5 pt-6 px-4">
                <button onclick="NetflixMatcher.instantPlanMovieDate('${item.title.replace(/'/g, "\\'")}', '${item.type}')" 
                    class="w-full bg-gradient-to-r from-[#eb459e] via-[#faa61a] to-[#5865F2] text-white py-3.5 rounded-xl font-black text-sm tracking-wide transition transform hover:scale-105 active:scale-95 shadow-xl hover:shadow-pink-500/30 flex items-center justify-center gap-2">
                    <i class="fas fa-magic"></i>
                    <span>1-CLICK: NAPLÁNOVAT NA DNEŠNÍ VEČER! 🍿</span>
                </button>
                <button onclick="NetflixMatcher.planMatchDate('${item.title.replace(/'/g, "\\'")}', '${item.type}')" 
                    class="w-full bg-white/10 hover:bg-white/15 text-gray-200 py-2.5 rounded-xl font-bold text-xs tracking-wide transition flex items-center justify-center gap-2">
                    <i class="far fa-calendar-alt"></i>
                    <span>Vybrat jiné datum a detaily 📅</span>
                </button>
                <button onclick="NetflixMatcher.closeMatchOverlay()" 
                    class="w-full py-2 text-gray-500 hover:text-white transition text-xs font-bold tracking-wider uppercase">
...
export async function instantPlanMovieDate(title, type) {
    const overlay = document.getElementById("tinder-match-overlay");
    if (overlay) overlay.remove();
    ...
    const newEvent = {
        id: 'date_' + Date.now(),
        title: `🍿 Filmový večer: ${title}`,
        date: todayStr,
        time: '20:00',
        type: 'date',
        description: `Společně vybraný ${type === 'game' ? 'herní večer' : 'film'} přes Tinder Matcher! 💖🎮`,
        created_at: new Date().toISOString()
    };

    const { state, saveStateToCache } = await import('@core/state.js');
    if (!state.calendarEvents) state.calendarEvents = [];
    state.calendarEvents.push(newEvent);
    saveStateToCache();

    const { safeUpsert } = await import('@core/offline.js');
    await safeUpsert('calendar_events', newEvent).catch(() => {});

    const { triggerConfetti, triggerHaptic } = await import('@core/utils.js');
    triggerConfetti();
    triggerHaptic('success');
    ...
    const { showNotification } = await import('@core/theme.js');
    showNotification(`Filmové rande „${title}“ bylo přidáno do Kalendáře na dnešek! 🍿❤️`, 'success');
}
...
export async function planMatchDate(title, type) {
    ...
    if (!window.Library) {
        await import('@domains/entertainment/library/index.js');
    }
