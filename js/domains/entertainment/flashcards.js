...
function renderCard() {
    ...
    window.fcNext = async (result) => {
        ...
        const cardData = cards[currentIndex];
        import('@domains/entertainment/spaced-repetition.js').then(sr => sr.saveReview(cardData.id, result));
...
function finishDuel() {
    ...
    if (winner !== 'Remíza' && winner.toLowerCase() === state.currentUser?.name.toLowerCase()) {
        import('@domains/entertainment/achievements.js').then(a => a.autoUnlock('matura_quiz_winner'));
        triggerConfetti(stage);
    }
