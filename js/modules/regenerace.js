...
if (!document.getElementById('reg-animations')) {
    const style = document.createElement('style');
    style.id = 'reg-animations';
    style.innerHTML = `
        @keyframes reg-pulse-glow {
            0% { box-shadow: 0 0 5px rgba(255, 255, 255, 0.1); transform: scale(1); }
            50% { box-shadow: 0 0 25px rgba(255, 255, 255, 0.2); transform: scale(1.05); }
            100% { box-shadow: 0 0 5px rgba(255, 255, 255, 0.1); transform: scale(1); }
        }
        @keyframes reg-line-flow {
            0% { opacity: 0.4; }
            50% { opacity: 1; }
            100% { opacity: 0.4; }
        }
        .animate-reg-glow {
            animation: reg-pulse-glow 3s infinite ease-in-out;
        }
        .animate-reg-flow {
            animation: reg-line-flow 2s infinite ease-in-out;
        }
    `;
...
function renderHolisticAnalysis(content) {
    ...
    return `
        ...
        <div class="glass-card bg-gradient-to-br ${theme.bg} rounded-3xl border ${theme.border} overflow-hidden shadow-2xl transition-all duration-300 hover:border-emerald-500/40 relative group/sec">
    ...
    const html = `
        <div class="h-full overflow-y-auto no-scrollbar bg-[#36393f] relative pb-12">
            ...
            <div class="max-w-2xl mx-auto px-4 pt-4 pb-12 space-y-4 animate-fade-in">
                ...
                <!-- Hero Section -->
                <div class="glass-card bg-gradient-to-br from-[#1a0b2e] to-[#0f0a1a] rounded-3xl border border-purple-500/20 overflow-hidden shadow-2xl transition-all duration-300 hover:border-purple-500/40 relative group/sec">
                                                                ...
                                                                <div class="space-y-3 pt-1">
                                                                    ${parsed.sections.map(sec => `
                                                                        <div class="space-y-1 group/sec">
                                                                            ...
                                                                            <p class="text-[12px] text-gray-300 leading-relaxed font-medium group-hover/sec:text-white transition-colors">
        ...
        const theme = SUPPLEMENT_THEMES[section.supplementId] || { bg: 'from-[#111a2a] to-[#0a0f1a]', accent: 'text-indigo-400', border: 'border-indigo-500/20', iconBg: 'bg-indigo-500/10' };
        const accentColor = theme.accent.replace('text-', '');
        const isCollapsed = collapsedScienceSections[sIdx] !== false; // Default true
                                        ...
                                        <div class="bg-white/[0.02] p-6 rounded-2xl border border-white/5 hover:border-${accentColor}/20 transition-all duration-300 group/item">
                                            <div class="flex items-start gap-5">
                                                ...
                                                <div class="flex-1 space-y-3">
                                                    ...
                                                    <p class="text-[13px] text-gray-300 leading-relaxed font-medium">${item.text}</p>
                                                    <div class="bg-${accentColor}/5 p-4 rounded-xl border-l-2 border-${accentColor}/30">
                                                        <p class="text-[13px] text-gray-200 font-bold leading-relaxed">${item.result}</p>
                                                    </div>
