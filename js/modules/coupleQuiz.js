...
let quizQuestions = []; // questions while creating
...
function renderHub(quizzes) {
    ...
    const otherQuizzes = quizzes.filter(q => q.creator_id !== state.currentUser.id);

    return `
    <!-- Create Button -->
    <button onclick="import('./js/modules/coupleQuiz.js').then(m => m.startCreate())"
        class="w-full py-4 bg-gradient-to-r from-[#5865F2] to-[#eb459e] text-white font-black rounded-xl shadow-lg hover:shadow-[#5865F2]/30 transition transform hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-2">
    ...
    <div>
        ...
        ` : myQuizzes.map(quiz => {
            const partnerAnswer = quiz.couple_quiz_answers?.find(a => a.answerer_id !== state.currentUser.id);
            const totalQ = Array.isArray(quiz.questions) ? quiz.questions.length : 0;
            return `
            <div class="bg-[#2f3136] rounded-xl border border-[#202225] shadow-md overflow-hidden mb-3">
                <div class="p-4 flex items-center gap-4">
                    ...
                    ${partnerAnswer
                        ? `<div class="text-right">
                            <div class="text-lg font-black text-[#3ba55c]">${partnerAnswer.score}/${partnerAnswer.total}</div>
                            <div class="text-[10px] text-gray-500">partner</div>
                           </div>`
                        : `<span class="text-[10px] text-gray-500 italic">Čeká na odpověď...</span>`
                    }
    ...
    <div class="bg-[#2f3136] rounded-xl border border-[#202225] overflow-hidden shadow-xl">
        <div class="bg-gradient-to-r from-[#5865F2]/20 to-[#faa61a]/10 p-4 border-b border-[#202225]">
            <h2 class="font-bold text-white flex items-center gap-2">
                <i class="fas fa-puzzle-piece text-[#5865F2]"></i> Nový Kvíz
            </h2>
            <p class="text-xs text-gray-400 mt-1">Napiš otázky o sobě – partner bude hádat správné odpovědi</p>
        </div>

        <div class="p-5 space-y-5">
            <div>
                <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Název kvízu</label>
                <input type="text" id="quiz-title" placeholder="Co víš o mně?" maxlength="60"
            ...
            <div>
                ...
                <div id="questions-container" class="space-y-3">
                    ...
                    ` : quizQuestions.map((q, i) => `
                        <div class="bg-[#202225] rounded-xl p-3 border border-[#40444b]">
                            <div class="flex items-start gap-3">
                                <div class="w-6 h-6 bg-[#5865F2] rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0 mt-0.5">${i + 1}</div>
                                <div class="flex-1 space-y-2">
                                    <input type="text" value="${q.question}" placeholder="Tvá otázka..."
                                        ...
                                        class="w-full bg-[#2f3136] text-white text-sm p-2 rounded border border-[#40444b] outline-none focus:border-[#5865F2] transition">
                                    ...
                                    <p class="text-[10px] text-gray-600">↑ Tuhle odpověď partner uvidí po vyhodnocení</p>
                                </div>
            ...
            <div class="grid grid-cols-2 gap-3 pt-2">
                ...
                <button onclick="import('./js/modules/coupleQuiz.js').then(m => m.saveQuiz())"
                    ...
                    <i class="fas fa-check mr-2"></i> Uložit kvíz
                </button>
            </div>
        </div>
    </div>`;
...
export function addQuestion() {
    quizQuestions.push({ question: '', my_answer: '' });
    renderCoupleQuiz();
}
...
export async function saveQuiz() {
    ...
    const validQuestions = quizQuestions.filter(q => q.question.trim() && q.my_answer.trim());
    ...
    try {
        const { error } = await supabase.from('couple_quizzes').insert({
        ...
        if (window.showNotification) window.showNotification('Kvíz vytvořen! 🧠', 'success');
        quizView = 'hub';
        quizQuestions = [];
        renderCoupleQuiz();
    } catch (err) {
...
export async function nextQuestion() {
    ...
    currentAnswers[currentQuestionIdx] = answer;
    ...
    if (currentQuestionIdx < currentQuiz.questions.length - 1) {
        ...
        questions.forEach((q, i) => {
            ...
            if (correct === given || correct.includes(given) || given.includes(correct)) {
                score++;
            }
        });

        // Save to Supabase
        await supabase.from('couple_quiz_answers').upsert({
            quiz_id: currentQuiz.id,
            answerer_id: state.currentUser.id,
            answers: currentAnswers,
            score,
            total: questions.length
        }, { onConflict: 'quiz_id,answerer_id' });
